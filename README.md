...
- [microautoware_micro-ROS_stm32: micro-ROS for microAutoware using STM32CubeMX/IDE
](#microautoware-autoware-vehicle-interface-with-micro-ros-for-stm32cubemxide)
...
5. Modify the generated `Makefile` to include the following code **before the `build the application` section**:
   ...
   # micro-ROS addons
   #######################################
   LDFLAGS += microautoware_micro-ROS_stm32/microros_static_library/libmicroautoware/libmicroautoware.a
   C_INCLUDES += -Imicroautoware_micro-ROS_stm32/microros_static_library/libmicroautoware/microros_include

   # Add micro-ROS utils
   C_SOURCES += microautoware_micro-ROS_stm32/extra_sources/custom_memory_manager.c
   C_SOURCES += microautoware_micro-ROS_stm32/extra_sources/microros_allocators.c
   C_SOURCES += microautoware_micro-ROS_stm32/extra_sources/microros_time.c

   # Set here the custom transport implementation
   C_SOURCES += microautoware_micro-ROS_stm32/extra_sources/microros_transports/dma_transport.c
...
6. Execute the static library generation tool. Compiler flags will retrieved automatically from your `Makefile` and user will be prompted to check if they are correct.
   ...
   docker run -it --rm -v $(pwd):/project --env MICROROS_LIBRARY_FOLDER=microautoware_micro-ROS_stm32/microros_static_library microros/micro_ros_static_library_builder:humble
   ```
...
2. Go to `Project -> Settings -> C/C++ Build -> Settings -> Build Steps Tab` and in `Pre-build steps` add:
   ...
   ```bash
   docker pull microros/micro_ros_static_library_builder:humble && docker run --rm -v ${workspace_loc:/${ProjName}}:/project --env MICROROS_LIBRARY_FOLDER=microautoware_micro-ROS_stm32/microros_static_library_ide microros/micro_ros_static_library_builder:humble
   ```

3. Add micro-ROS include directory. In `Project -> Settings -> C/C++ Build -> Settings -> Tool Settings Tab -> MCU GCC Compiler -> Include paths` add `microautoware_micro-ROS_stm32/microros_static_library_ide/libmicroautoware/include`
4. Add the micro-ROS precompiled library. In `Project -> Settings -> C/C++ Build -> Settings -> MCU GCC Linker -> Libraries`
      - add `<ABSOLUTE_PATH_TO>/microautoware_micro-ROS_stm32/microros_static_library_ide/libmicroautoware` in `Library search path (-L)`
